...
class SimonettaController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontend.viste.mensile.costi_annuale_mesi')
...
//            ->orderBy('tx_mask_p_data_partenza', 'ASC')
    ...
    public function indexAnno()
    {
        $message = 'Vista in costruzione';
        $houses_typo = $this->getHousesAbbrArray();
        return view('frontend.viste.mensile.costi_annuale_anno')
            ->with(compact('houses_typo'))
            ->with(compact('message'));
    }

    public function getDataTablesYears(Request $request)
    {
        $year = $request->year ? $request->year : now()->year;
        $house = $request->house ? $request->house : NULL;

        $data = Booking::select([
            Booking::raw('YEAR(tx_mask_p_data_arrivo) as years'),
            Booking::raw('MONTHNAME(STR_TO_DATE(MONTH(tx_mask_p_data_arrivo), "%m")) as month'),
            Booking::raw('SUM(costi_check_in_self_check_in) as costi_check_in_self_check_in'),
            Booking::raw('SUM(tx_mask_t3_p_s_extra_checkin) as tx_mask_t3_p_s_extra_checkin'),
            Booking::raw('SUM(totale_pulizie) as totale_pulizie'),
            ...
            ->orderBy('years', 'DESC')
//            ->orderBy('tx_mask_p_data_partenza', 'ASC')
            ->groupBy(Booking::raw('YEAR(tx_mask_p_data_arrivo)'))
            ->get();
        ...
        return Datatables::of($data)
            ...
            ->rawColumns([
                'year', 'costo_cin', 'totale_pulizie', 'supervisor_pulizie', 'costo_co', 'costi_costo_operatore_cambio_biancheria',
                'tx_mask_t3_p_s_ex_checkout', 'tx_mask_p_perc_importo_fisso', 'totale_biancheria', 'sum_row',
                'sum_costo_cin', 'sum_tot_pulizie', 'sum_tot_costo_co' , 'sum_tot_op_cambio', 'sum_tot_extra_co',
                'sum_tot_commissioni_sitiweb', 'sum_tot_supervisor_pulizie', 'sum_tot_totale_biancheria', 'sum_tot_totale_row'
            ])
    ...
    public function getDataCostiSpeseMacroYears(Request $request)
    {
        $year = $request->year ? $request->year : now()->year;
        $house = $request->house ? $request->house : NULL;

        $data = Typo::select([
            Typo::raw('YEAR(tx_mask_sp_data) as years'),
            Typo::raw('SUM(tx_mask_db_importo) as tx_mask_db_importo'),
            Typo::raw('SUM(tx_mask_t3_p_s_extra_checkin) as tx_mask_t3_p_s_extra_checkin'),
            Typo::raw('SUM(totale_pulizie) as totale_pulizie'),
            Typo::raw('SUM(tx_mask_t3_p_extra_p) as tx_mask_t3_p_extra_p'),
            Typo::raw('SUM(costo_co) as costo_co'),
            Typo::raw('SUM(costi_costo_operatore_cambio_biancheria) as costi_costo_operatore_cambio_biancheria'),
            Typo::raw('SUM(tx_mask_t1_op_costo_extra_cambio_biancheria) as tx_mask_t1_op_costo_extra_cambio_biancheria'),
            Typo::raw('SUM(tx_mask_t3_p_s_ex_checkout) as tx_mask_t3_p_s_ex_checkout'),
            Typo::raw('(SUM(costi_costo_kit) + SUM(costi_costo_cambi)) as totale_biancheria')
        ])
            ->when($house, function ($q, $house){
                return $q->whereIn('tx_mask_p_casa', $house);
            })
            ...
            ->orderBy('years', 'ASC')
//            ->orderBy('tx_mask_p_data_partenza', 'ASC')
            ->groupBy(Typo::raw('YEAR(tx_mask_sp_data)'))
            ->get();
